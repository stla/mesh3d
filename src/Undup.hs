...
import           Data.List
import           Data.Maybe
import           Data.Vector.Unboxed         (Unbox, Vector, freeze, (!), unsafeFreeze, fromList)
...
unique'' :: forall a . (Unbox a, Eq a) => Vector a -> IO (Vector a, Vector Int)
unique'' vs = do
  let n = VU.length vs
  idx <- VM.replicate n 0 :: IO (IOVector Int)
  visited <- VM.replicate n False :: IO (IOVector Bool)
...
vslong :: [Double]
vslong = concat (replicate 1000 vs) ++ [10,11,12]

vslong' :: Vector Double
vslong' = VU.fromList vslong
